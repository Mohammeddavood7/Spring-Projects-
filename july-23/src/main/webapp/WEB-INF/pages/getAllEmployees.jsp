	...
	<div class="container">
		...
		<form name="frm">
			...
			<div class="m-1">
				...
				</select> <input class="w-100 form-control" name="search_value"
					id="search_value">
			...
			<table>
				...
				<%
				List<Employee> em = (List<Employee>) request.getAttribute("emp");
				if (em == null || em.isEmpty()) {
				%>
				<tr>
					<td colspan='6' style='text-align: center'>No Employee Record
						Available</td>
				</tr>
				<%
				} else {
				...
				%>
				<tr>
					<td><%=em.get(i).getEmpId()%></td>
					<td><%=em.get(i).getEmpName()%></td>
					<td><%=em.get(i).getEmpAddress()%></td>
					<td><%=em.get(i).getEmpSal()%></td>
					<td><button type="button" class="edit"
							onclick="editEmployee(<%=em.get(i).getEmpId()%>)">Edit</button></td>
					<td><button type="button" class="delete"
							onclick="deleteEmployee(<%=em.get(i).getEmpId()%>)">Delete</button></td>
				</tr>
				<%
				}
				}
